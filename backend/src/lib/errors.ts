...
const formatRequest = ({ id, body, query, method, headers, protocol, url, hostname, ip, user }: FastifyRequest) => ({
    id,
    hostname,
    ip,
    headers,
        ...
        .setErrorHandler((error, req, res) =>
            Boom.isBoom(error)
                ? res
                      ...
                      .log.error({ error: { stack: error.stack, ...error.output }, request: formatRequest(req) })
                : res
                      .status(500)
