...
function getColorContrast(color: Color, ratio = 0.8) {
  return color.isDark() ? color.lighten(ratio) : color.darken(ratio);
}

function generateCssColorVariables(colors: Record<string, Color>) {
  return Object.entries(colors).reduce<Record<string, string>>((acc, [key, color]) => {
